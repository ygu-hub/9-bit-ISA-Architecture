...
.find_non_space
    movi r1 1
    # TODO: loop through encode message pointer from 64 - 127, decode until the result is non zero, then branch to loop
    # r7 = 201, r1 = mem[201] = encrypted msg
    # line 119 on machine code
    mov r7 r0
    add r7 r1
    ldr r1 [r7]
    ldr r1 [r1]

    # r5 = decrypted message
    xor r1 r2
    mov r5 r1
    ...
    # Update encode message pointer, r7 = 201
    movi r1 1
    mov r7 r0
    add r7 r1
    ldr r4 [r7]
...
.exit
    # update encode message pointer end mem[202] to mem[197] = 127, r7 initially 201
    movi r1 1
    mov r7 r0
    sub r7 r1
    sub r7 r1
...
# sp - 1 = 128
# sp + 0 = write pointer (starting at 0)
# sp + 1 = encode message pointer (starting at position with non-space character)
# sp + 2 = encode message pointer end (always 127)
.loop
    # r7 = 201, r4 = mem[201] = encrypted msg
    mov r7 r0
    add r7 r1
    ldr r4 [r7]
    ldr r4 [r4]
    ...
    # TODO: check parity of r1, if it is 1, store 0x80(error flag), else store as normal
    # r7 = 196, r1 = 256

    # get pointer r7 = sp-4 (contains 1111_1111) and r6 = sp-1 (contains parity bit)
    movi r1 1
    mov r7 r0
    sub r7 r1
    sub r7 r1
    ...
    # extract parity from message(r4) by bitwise and r1 = 1111_1111
    and r4 r1

    # load parity bit from (r6) sp - 1
    ldr r1 [r6]
    ldr r5 [r1]
    # r1 = 0x80
    movi r6 0
    ...
    # store error flag 0x80 into DM[X]
    ldr r5 [r0]
    str r1 [r5]
    b 13

# store the decrypted char into DM[X]
.else
...
.end_encoding
    # r2 = update next lfsr state, r7 = 199
    movi r1 1
    mov r7 r0
    sub r7 r1
